...
def calculate_cagr(start_value, end_value, num_years):
    ...
    if start_value is None or end_value is None or num_years <= 0 or start_value <= 0:
        return None
    try:
        base = end_value / start_value
        if base < 0: # CAGR is not well-defined for negative growth over multiple periods
            return None
        return base**(Decimal(1)/num_years) - 1
...
def screen_stocks(config_file, output_csv):
    ...
    try:
        ...
        for company in companies:
            ...
            # Condition 2: Revenue growth (CAGR)
            min_revenue_cagr_percent = config.get('min_revenue_cagr_percent')
            if min_revenue_cagr_percent is not None:
                oldest_revenue = oldest_report.get('revenue')
                latest_revenue = latest_report.get('revenue')
                revenue_cagr = calculate_cagr(oldest_revenue, latest_revenue, num_years)
                if revenue_cagr is None or revenue_cagr * 100 < min_revenue_cagr_percent:
                    logging.debug(f"Skipping {ticker}: Revenue CAGR ({revenue_cagr*100 if revenue_cagr is not None else 'N/A'}%) below {min_revenue_cagr_percent}%.")
                    continue

            # Condition 3: Operating income growth (CAGR)
            min_operating_income_cagr_percent = config.get('min_operating_income_cagr_percent')
            if min_operating_income_cagr_percent is not None:
                oldest_operating_income = oldest_report.get('operating_income')
                latest_operating_income = latest_report.get('operating_income')
                operating_income_cagr = calculate_cagr(oldest_operating_income, latest_operating_income, num_years)
                if operating_income_cagr is None or operating_income_cagr * 100 < min_operating_income_cagr_percent:
                    logging.debug(f"Skipping {ticker}: Operating Income CAGR ({operating_income_cagr*100 if operating_income_cagr is not None else 'N/A'}%) below {min_operating_income_cagr_percent}%.")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='growth_stocks.csv', help='Output CSV file name.')
